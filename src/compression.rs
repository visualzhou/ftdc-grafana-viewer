...
impl Compression {
    ...
    pub fn decompress_ftdc(data: &[u8], reference_values: Option<&[u64]>) -> Result<Vec<u64>, FtdcError> {
        ...
        let uncompressed_size = if data.len() >= 4 {
            ...
            let size = rdr.read_u32::<LittleEndian>()
                .map_err(|e| FtdcError::Compression(format!("Failed to read uncompressed size: {}", e)))?;
            
            // Sanity check for reasonable size
            if size > 100 * 1024 * 1024 { // 100 MB max
                return Err(FtdcError::Compression(format!("Unreasonable uncompressed size: {} bytes", size)));
            }
            
            size
        } else {
            return Err(FtdcError::Compression("Invalid FTDC data: too short".to_string()));
        };
        ...
        while i < decompressed.len() {
            ...
            if value_count > 10_000_000 { // 10 million values max
                return Err(FtdcError::Compression(format!("Too many values decoded: {}", value_count)));
            }
        }
        ...
        while i < decoded_values.len() {
            ...
            if value == 0 && i + 1 < decoded_values.len() {
                ...
                // Safety check
                if count > 10_000_000 { // Max 10 million consecutive zeros
            ...
            // Safety check
            if expanded_values.len() > 100_000_000 { // 100 million expanded values max
                return Err(FtdcError::Compression(format!("Too many expanded values: {}", expanded_values.len())));
            }
        }
        
        println!("Expanded to {} values after RLE decoding", expanded_values.len());
...
mod tests {
    ...
    fn test_reference_values() {
        ...
        let decompressed = Compression::decompress_ftdc(&compressed, Some(&reference_values)).unwrap();
